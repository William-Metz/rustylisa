...
pub fn CalculateAmplitudes(wave_builder: &Wave_Builder) -> [f64; 248] {
    // Calculate some useful trig functions of angle lota

    // Input stuff
    let lotaDN: f64 = wave_builder.lotaDN; 
    let beta_: f64 = wave_builder.beta_;
    let delta: f64 = wave_builder.delta;
    let eta: f64 = wave_builder.eta;
    let pi: f64 = PI;
    let chiaxDN: f64 = wave_builder.chiaxDN;
    let chiayDN: f64 = wave_builder.chiayDN;
    let chiazDN: f64 = wave_builder.chiazDN;
    let chisxDN: f64 = wave_builder.chisxDN;
    let chisyDN: f64 = wave_builder.chisyDN;
    let chiszDN: f64 = wave_builder.chiszDN;
