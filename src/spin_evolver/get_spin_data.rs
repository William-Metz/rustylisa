...
// Created: 2024
// File Path: src/spin_evolver/get_spin_data.rs
// ------------------------------------------------------------
...
impl SpinEvolverClass {
    pub fn get_spin_data_at_time(&mut self, tau: f64) -> Option<SpinData> {
        if self.test_case.chi1 == 0.0 && self.test_case.chi2 == 0.0 {
            ...
            data.psi = self.test_case.lambda0 + tmp
                - (6.0 * v_for_tau.powi(3) * (v_for_tau / self.v_0).ln());
            Some(data)
        } else {
            ...
            data.chi_ax =
                0.5 * (f_n * (self.chi1x_n - self.chi2x_n) + f_p * (self.chi1x_p - self.chi2x_p));
            data.chi_ay =
                0.5 * (f_n * (self.chi1y_n - self.chi2y_n) + f_p * (self.chi1y_p - self.chi2y_p));
            data.chi_az =
                0.5 * (f_n * (self.chi1z_n - self.chi2z_n) + f_p * (self.chi1z_p - self.chi2z_p));
            data.chi_sx =
                0.5 * (f_n * (self.chi1x_n + self.chi2x_n) + f_p * (self.chi1x_p + self.chi2x_p));
            data.chi_sy =
                0.5 * (f_n * (self.chi1y_n + self.chi2y_n) + f_p * (self.chi1y_p + self.chi2y_p));
            ...
            let v_for_tau = self.v_calc.v_at_time(tau);
            data.v = v_for_tau;
            data.psi = self.test_case.lambda0
                + self.v_calc.psi_orb_for_last_v()
                + f_n * self.psi_pr_n
                + f_p * self.psi_pr_p
                - 6.0 * v_for_tau.powi(3) * (v_for_tau / self.v_0).ln();
            Some(data)
