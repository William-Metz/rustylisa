...
impl SpinEvolverClass {
    pub fn do_step_succeeded(&mut self) -> bool {
        ...
        let chi1x_dot_n: f64 =
            omega_1 * (self.ell_y_n * self.chi1z_n - self.ell_z_n * self.chi1y_n);
        let chi1y_dot_n: f64 =
            omega_1 * (self.ell_z_n * self.chi1x_n - self.ell_x_n * self.chi1z_n);
        let chi1z_dot_n: f64 =
            omega_1 * (self.ell_x_n * self.chi1y_n - self.ell_y_p * self.chi1x_n);
        let omega_2: f64 = v5 * (self.c_20 + self.c_22 * v2 + self.c_24 * v4) / l_n;
        let chi2x_dot_n: f64 =
            omega_2 * (self.ell_y_n * self.chi2z_n - self.ell_z_n * self.chi2y_n);
        let chi2y_dot_n: f64 =
            omega_2 * (self.ell_z_n * self.chi2x_n - self.ell_x_n * self.chi2z_n);
        let chi2z_dot_n: f64 =
            omega_2 * (self.ell_x_n * self.chi2y_n - self.ell_y_n * self.chi2x_n);
        ...
        let ell_dot_n: f64 = self.eta
            * (-1.0 / v2 + self.l_2 + 2.0 * self.l_2 * v1 + 3.0 * self.l_4 * v2)
            * self.v_calc.v_dot_for_last_v()
            / l_n;
        let ell_x_dot_n: f64 = -self.op_delta2_i4 * chi1x_dot_n - self.om_delta2_i4 * chi2x_dot_n
            + ell_dot_n * self.ell_x_n;
        let ell_y_dot_n: f64 = -self.op_delta2_i4 * chi1y_dot_n - self.om_delta2_i4 * chi2y_dot_n
            + ell_dot_n * self.ell_y_n;
        let ell_z_dot_n: f64 = -self.op_delta2_i4 * chi1z_dot_n - self.om_delta2_i4 * chi2z_dot_n
            + ell_dot_n * self.ell_z_n;
        ...
        let alpha_dot_n: f64 = (self.ell_y_n * ell_x_dot_n - self.ell_x_n * ell_y_dot_n)
        ...
        if ell_y_f < 0.0 && self.ell_y_n > 0.0 {
            ...
            && self.ell_y_n < 0.0
            && (ell_x_f * self.ell_y_n - self.ell_x_n * ell_y_f) / (self.ell_y_n - ell_y_f) < 0.0
        {
            self.n_alpha_cycles = -1
        }
        ...
        self.ell_n = atan2(
            self.ell_z_n,
            (self.ell_x_n * self.ell_x_n + self.ell_y_n * self.ell_y_n).sqrt(),
        );

        // Calculate the ideal next time step
        let s1dot: f64 =
            (chi1x_dot_n * chi1x_dot_n + chi1y_dot_n * chi1y_dot_n + chi1z_dot_n * chi1z_dot_n)
                .sqrt();
        let s2dot: f64 =
            (chi2x_dot_n * chi2x_dot_n + chi2y_dot_n * chi2y_dot_n + chi2z_dot_n * chi2z_dot_n)
                .sqrt();
        let delta_tau: f64 = f64::min(self.test_case.chi1 / s1dot, self.test_case.chi2 / s2dot);
        if delta_tau < self.delta_tau_hf {
            // if we need a smaller step
            if self.delta_tau_hp > self.delta_tau_hf {
            ...
            // if we don't need a smaller step, repeat the current step
